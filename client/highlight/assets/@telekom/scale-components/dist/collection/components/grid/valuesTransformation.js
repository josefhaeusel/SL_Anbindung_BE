export function createCssString(setProp) {
...
export function isBreakpointValuesEmpty(values) {
  let isEmpty = true;
...
export function fillEmptyBreakpointValues(values) {
...
export function transformBreakpointValuesData(values) {
  return values.reduce((a, v) => (Object.assign(Object.assign({}, a), { [v.breakpointName]: v.value })), {});
}
export function propsToBreakpointValuesArray(values) {
  return [
...
export function createBreakpointValuedProp(propName, values) {
  if (!propName || !values) {
    return;
  }
  return {
