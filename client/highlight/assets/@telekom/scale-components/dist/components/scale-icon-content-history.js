...
const ContentHistory = /*@__PURE__*/ proxyCustomElement(class extends HTMLElement {
  constructor() {
    super();
    this.__registerHost();
...
function defineCustomElement$1() {
  ...
  const components = ["scale-icon-content-history"];
  components.forEach(tagName => { switch (tagName) {
    case "scale-icon-content-history":
      if (!customElements.get(tagName)) {
        customElements.define(tagName, ContentHistory);
      }
      break;
  } });
}

const ScaleIconContentHistory = ContentHistory;
const defineCustomElement = defineCustomElement$1;

export { ScaleIconContentHistory, defineCustomElement };
