...
const ServiceOneTimePassword = /*@__PURE__*/ proxyCustomElement(class extends HTMLElement {
  constructor() {
    super();
    this.__registerHost();
...
}, [0, "scale-icon-service-one-time-password", {
    "size": [514],
    "fill": [1],
    "color": [1],
    "selected": [516],
...
function defineCustomElement$1() {
  ...
  const components = ["scale-icon-service-one-time-password"];
  components.forEach(tagName => { switch (tagName) {
    case "scale-icon-service-one-time-password":
      if (!customElements.get(tagName)) {
        customElements.define(tagName, ServiceOneTimePassword);
      }
      break;
  } });
}

const ScaleIconServiceOneTimePassword = ServiceOneTimePassword;
const defineCustomElement = defineCustomElement$1;

export { ScaleIconServiceOneTimePassword, defineCustomElement };
