...
const WeatherOvercast = /*@__PURE__*/ proxyCustomElement(class extends HTMLElement {
  constructor() {
    super();
    this.__registerHost();
...
function defineCustomElement$1() {
  ...
  const components = ["scale-icon-weather-overcast"];
  components.forEach(tagName => { switch (tagName) {
    case "scale-icon-weather-overcast":
      if (!customElements.get(tagName)) {
        customElements.define(tagName, WeatherOvercast);
      }
      break;
  } });
}

const ScaleIconWeatherOvercast = WeatherOvercast;
const defineCustomElement = defineCustomElement$1;

export { ScaleIconWeatherOvercast, defineCustomElement };
