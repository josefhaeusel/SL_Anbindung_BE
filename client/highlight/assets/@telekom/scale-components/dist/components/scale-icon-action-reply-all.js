...
const ActionReplyAll = /*@__PURE__*/ proxyCustomElement(class extends HTMLElement {
  constructor() {
    super();
    this.__registerHost();
...
function defineCustomElement$1() {
  ...
  const components = ["scale-icon-action-reply-all"];
  components.forEach(tagName => { switch (tagName) {
    case "scale-icon-action-reply-all":
      if (!customElements.get(tagName)) {
        customElements.define(tagName, ActionReplyAll);
      }
      break;
  } });
}

const ScaleIconActionReplyAll = ActionReplyAll;
const defineCustomElement = defineCustomElement$1;

export { ScaleIconActionReplyAll, defineCustomElement };
