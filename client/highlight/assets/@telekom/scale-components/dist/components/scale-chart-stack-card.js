...
const ChartStackCard = /*@__PURE__*/ proxyCustomElement(class extends HTMLElement {
  constructor() {
    super();
    this.__registerHost();
    this.readData = (data) => {
...
function defineCustomElement$1() {
  ...
  const components = ["scale-chart-stack-card", "scale-card"];
  components.forEach(tagName => { switch (tagName) {
    case "scale-chart-stack-card":
      if (!customElements.get(tagName)) {
        customElements.define(tagName, ChartStackCard);
      }
      break;
    case "scale-card":
      if (!customElements.get(tagName)) {
...
const ScaleChartStackCard = ChartStackCard;
const defineCustomElement = defineCustomElement$1;

export { ScaleChartStackCard, defineCustomElement };
