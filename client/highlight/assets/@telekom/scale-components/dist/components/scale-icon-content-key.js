...
const ContentKey = /*@__PURE__*/ proxyCustomElement(class extends HTMLElement {
  constructor() {
    super();
    this.__registerHost();
...
function defineCustomElement$1() {
  ...
  const components = ["scale-icon-content-key"];
  components.forEach(tagName => { switch (tagName) {
    case "scale-icon-content-key":
      if (!customElements.get(tagName)) {
        customElements.define(tagName, ContentKey);
      }
      break;
  } });
}

const ScaleIconContentKey = ContentKey;
const defineCustomElement = defineCustomElement$1;

export { ScaleIconContentKey, defineCustomElement };
