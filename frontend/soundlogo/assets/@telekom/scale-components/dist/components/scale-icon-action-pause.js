...
const ActionPause = /*@__PURE__*/ proxyCustomElement(class extends HTMLElement {
  constructor() {
    super();
    this.__registerHost();
...
function defineCustomElement$1() {
  ...
  const components = ["scale-icon-action-pause"];
  components.forEach(tagName => { switch (tagName) {
    case "scale-icon-action-pause":
      if (!customElements.get(tagName)) {
        customElements.define(tagName, ActionPause);
      }
      break;
  } });
}

const ScaleIconActionPause = ActionPause;
const defineCustomElement = defineCustomElement$1;

export { ScaleIconActionPause, defineCustomElement };
