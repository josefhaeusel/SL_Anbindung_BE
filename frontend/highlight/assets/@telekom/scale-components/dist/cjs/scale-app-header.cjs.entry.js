...
const Header = class {
  ...
  menuMain() {
    ...
    return (index.h("ul", { class: "main-navigation", onKeyDown: (e) => {
      ...
      } }, this.hasSlotMenuMain ? (index.h("slot", { name: "menu-main" })) : (readData(this.mainNavigation).map((item) => (index.h("scale-nav-main", { href: item.href, active: isActive(item), megaMenuVisible: this.visibleMegaMenu === item.id, innerId: item.id, onMouseEnter: () => {
  ...
  menuIcon() {
    ...
    return (index.h("ul", { class: "meta-navigation" }, this.hasSlotMenuIcon ? (index.h("slot", { name: "menu-icon" })) : (readData(this.iconNavigation)
      ...
      } }, item.name)))), readData(this.userNavigation).length > 0 && (index.h("span", null, index.h("span", { class: "header__user-menu--desktop", "aria-label": this.userMenuAriaLabel }, index.h("scale-menu-flyout", null, index.h("scale-nav-icon", { slot: "trigger", active: this.userMenu, icon: 'user-file-user', refUserMenuToggle: (el) => (this.userMenuToggle = el), badge: badge, badgeLabel: badgeLabel, onKeyDown: (event) => {
        // Handle Spacebar separately because actual trigger is an <a>
        if (event.key === ' ') {
          event.target.click();
          event.preventDefault();
        }
      } }, shortName), index.h("scale-menu-flyout-list", null, index.h("app-navigation-user-menu", { hide: () => {
        if (!this.userMenu) {
          return;
        }
        this.userMenu = false;
      ...
      }, navigation: readData(this.userNavigation) })))), index.h("span", { class: "header__user-menu--mobile", "aria-label": this.userMenuAriaLabel }, index.h("scale-nav-icon", { slot: "trigger", active: this.userMenuMobile, icon: 'user-file-user', refMobileUserMenuToggle: (el) => (this.userMenuMobileToggle = el), clickLink: () => {
        this.mobileMenu = false;
  ...
  render() {
    return (index.h(index.Host, null, index.h("header", { class: "header__container" }, index.h("div", { class: this.getCssClassMap() }, index.h("div", { class: "header__brand" }, index.h("span", { class: "header__brand-before" }), index.h("span", { class: "header__brand-after" }), index.h("div", { class: "header__brand-content" }, index.h("div", { class: "header__brand-branding" }, this.hasSlotLogo ? (index.h("slot", { name: "logo" })) : (index.h("scale-logo", { transparent: true, href: this.logoHref, logoTitle: this.logoTitle, logoHideTitle: this.logoHideTitle, onClick: this.logoClick, variant: "white", scrollIntoViewOnFocus: true, focusable: true, styles: ":host { --logo-size: 36px;} @media (max-width: 1039px) { :host {--logo-size: 26px;} }", logoAriaDescribedBy: this.logoAriaDescribedBy, logoAriaHidden: this.scrolled }))), index.h("div", { class: "header__brand-sector" }, this.menuSector()), index.h("div", { class: "header__brand-meta" }, this.menuAddon()))), index.h("nav", { class: "header__nav", "aria-label": "top" }, index.h("span", { class: "header__nav-before" }), index.h("span", { class: "header__nav-after" }), index.h("div", { class: "header__nav-content" }, index.h("div", { class: "header__nav-logo" }, this.hasSlotLogoInverse ? (index.h("slot", { name: "logo-inverse" })) : (index.h("scale-logo", { transparent: true, language: "", href: this.logoHref, logoTitle: this.logoTitle, logoHideTitle: this.logoHideTitle, onClick: this.logoClick, focusable: this.scrolled || this.sticky, size: 24, logoAriaDescribedBy: this.logoAriaDescribedBy, logoAriaHidden: !this.scrolled }))), index.h("div", { class: "header__nav-menu-wrapper" }, index.h("div", { class: "header__nav-menu-main" }, this.menuMain()), index.h("div", { class: "header__nav-menu-icon" }, this.menuIcon())))), index.h("nav", { class: `header__nav__mobile-menu${this.mobileMenu ? ' header__nav__mobile-menu--opened' : ''}`, "aria-label": "main" }, this.hasSlotMenuMobile ? (index.h("slot", { name: "menu-mobile" })) : (index.h("div", null, index.h("app-navigation-sector-mobile", { navigation: readData(this.sectorNavigation), activeSectorId: this.activeSectorId, hide: () => {
